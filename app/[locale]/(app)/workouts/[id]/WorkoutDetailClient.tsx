  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="relative bg-foreground text-background pt-14 pb-6 px-4">
        ...
        <h1 className="text-2xl font-black leading-tight mb-1">{workout.title}</h1>
        <p className="text-background/60 text-sm">
          {workout.lessons_count} aulas · {workout.level}
        </p>
        <p className="text-background/50 text-sm mt-1">{workout.description}</p>

        {completedCount > 0 && (
          <div className="mt-4 space-y-1.5">
            <div className="flex justify-between text-xs text-background/60">
              <span>Em andamento</span>
              <span>{completedCount} / {workout.lessons_count}</span>
            </div>
        ...
        >
          {completedCount > 0 ? `Continuar — Aula ${completedCount + 1}` : 'Começar treino'}
        </button>
      </div>
